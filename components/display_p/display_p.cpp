...
void display_model_output(int output)
{
    ...
    M5.Display.printf(">> %s", label_for_output(output));
    M5.Display.endWrite();
}
...
{
    if (impact == nullptr || vibration == nullptr) return;
    ...
    for (int x = 0; x < impact_width; x++) {
        ...
        if (idx >= impact->length) idx = impact->length - 1;
    ...
    for (int x = 0; x < vibration_width; x++) {
        ...
        if (idx >= vibration->length) idx = vibration->length - 1;
