...
void edge_detection_task(void *vp_args)
{
    ...
    while (true)
    {
        if (xSemaphoreTake(hammer_edge_detect_smphr, portMAX_DELAY))
        {
            ...
            get_input_features(&inference_input);
...
{
#ifdef SINGLE_FILE_SAMPLE
    char filename[100];
    int file_count = 1;
    wav_data_t audio_sample;
    // check the number of files in the directory

#else
    char impact_filename[100];
    char vibration_filename[100];
    int file_count = 0;
#endif
    while (true)
    {
#ifdef SINGLE_FILE_SAMPLE
        sprintf(filename, MOUNT_POINT TEST_DIR "/sample_%d.wav", file_count);
        get_audio_sample(filename, &audio_sample);
        detect_hammer_edge_complete_samples(&audio_sample);
#else
        ...
        ESP_LOGI("Model", "output:%d", output);
#ifdef SINGLE_FILE_SAMPLE
        write_output_log(filename, filename, output);
        int32_t fileindex = file_count;
#else
        ...
        // this delay can be reduced or deleted.
