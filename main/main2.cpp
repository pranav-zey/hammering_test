...
void loop(void)
{
  ...
  if (M5.Mic.isEnabled())
  {
    ...
    while (M5.Mic.isRecording() < 2) { // Record till there is no space in queue
      M5.Mic.record(&(wav_data.wav[wav_idx]), WAVE_BLOCK_SIZE, SAMPLE_RATE);
...
extern "C" {
  ...
  void app_main()
  {
    setup();
    for (;;) {
      // xTaskCreate(loop, "LOOP_FUNCTION",4096, NULL,10,NULL);
      loop();
    }
  }
}